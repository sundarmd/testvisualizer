...
def get_api_credentials() -> Dict[str, str]:
    ...
    or prompt the user to enter it via sidebar input.

    Returns:
        Dict[str, str]: A dictionary containing the API key.
    """
    api_key = st.secrets.get("API_KEY") or st.sidebar.text_input(
...
def get_model_selection() -> str:
    """
    Provide a dropdown menu for users to select a language model.
...
def generate_and_validate_d3_code(
    ...
    """
    Generate, validate, and refine D3 code using OpenAI's API.
    """
    # Since only one API key and model are used, directly call the generate_d3_code function
    try:
        initial_code = generate_d3_code(df, api_key, user_input, viz_type)
        ...
        refined_code = refine_d3_code(cleaned_code, api_key)
        return refined_code
    except Exception as e:
        logger.error(f"Error generating and validating D3 code: {str(e)}")
        return generate_fallback_visualization()
            ...
            with st.expander("View/Edit Visualization Code"):
                ...
                with col3:
                    if st.button("Copy Code"):
                        ...
                        st.code(code_editor, language='javascript')
                        st.markdown(
                            f'<a href="data:text/javascript;charset=utf-8,{urllib.parse.quote(code_editor)}" download="visualization_code.js">Download Code</a>',
